...
export const useAdminPartners = () => {
  ...
  const fetchPartnerById = useCallback(async (id: any) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const verifyPartner = useCallback(async (id: any, isVerified: any) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const editPartner = async (partnerId: any, partnerData: any) => {
    try {
      setLoading(true);
  ...
  const removePartner = async (partnerId: any) => {
    try {
      setLoading(true);
