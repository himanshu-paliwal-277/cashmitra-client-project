...
const getBrandsByCategory = async (req, res) => {
  try {
    ...
    // Validate category
    if (!['mobile', 'tablet', 'laptop'].includes(category)) {
...
const calculatePrice = async (req, res) => {
  ...
  try {
    ...
    const ageInDays = Math.floor((currentDate - purchaseDateTime) / (1000 * 60 * 60 * 24));
    ...
    if (ageInMonths <= 6) {
      ...
      depreciationPercentage = ageInMonths * (product.depreciation.ratePerMonth * 1.5);
    } else if (ageInMonths <= 24) {
      // Standard depreciation for 6-24 months
      depreciationPercentage = 9 + (ageInMonths - 6) * product.depreciation.ratePerMonth;
    } else {
      // Slower depreciation after 24 months
      depreciationPercentage = 45 + (ageInMonths - 24) * (product.depreciation.ratePerMonth * 0.7);
    }
    ...
    const screenFactor = product.conditionFactors.screenCondition[screenCondition] / 100;
    const bodyFactor = product.conditionFactors.bodyCondition[bodyCondition] / 100;
    const batteryFactor = product.conditionFactors.batteryHealth[batteryHealth] / 100;
    const functionalFactor = product.conditionFactors.functionalIssues[functionalIssues] / 100;
    ...
    const marketDemandFactor = brandDemandFactors[product.brand] || 1.00;

    // Seasonal factor (higher demand during festive seasons)
    const currentMonth = currentDate.getMonth();
    let seasonalFactor = 1.00;
    if ([9, 10, 11].includes(currentMonth)) { // Oct, Nov, Dec - festive season
      seasonalFactor = 1.05;
    } else if ([3, 4, 5].includes(currentMonth)) { // Apr, May, Jun - summer season
      seasonalFactor = 0.98;
    }
    ...
    const storageValue = parseInt(product.variant.storage.replace(/[^0-9]/g, ''));
    let storageFactor = 1.00;
    ...
    const depreciatedPrice = product.basePrice * (1 - depreciationPercentage / 100);

    // Apply all factors
    let finalPrice = depreciatedPrice * weightedConditionFactor * marketDemandFactor * seasonalFactor * storageFactor;
    ...
    const minimumPrice = product.basePrice * 0.10;
    if (finalPrice < minimumPrice) {
      finalPrice = minimumPrice;
    }

    // Calculate confidence score based on data quality
    const confidenceScore = Math.min(95, 70 + 
...
const generatePriceRecommendations = (product, condition) => {
  ...
  if (condition.screenCondition === 'cracked' || condition.screenCondition === 'majorScratches') {
  ...
  if (condition.functionalIssues === 'major' || condition.functionalIssues === 'notWorking') {
    recommendations.push({
      type: 'repair',
...
const createSellOrder = async (req, res) => {
  ...
  try {
    ...
    if (!partnerShop) {
      return res.status(404).json({ message: 'No partner shop available in your area' });
    }
...
const submitAssessment = async (req, res) => {
  try {
    ...
    const brandKey = Object.keys(brandMultipliers).find(key => 
    ...
    if (answers.functionality) {
      ...
      conditionMultiplier *= functionalMultipliers[answers.functionality] || 0.8;
    }
    ...
    res.json({
      ...
      priceQuote: {
        ...
        marketInsights: {
          demand: 'high',
          brandRetention: brandKey && brandMultipliers[brandKey] > 1.2 ? 'excellent' : 'good',
          expectedSaleDays: '2-3'
        }
      },
      deviceInfo: {
...
const findProductsByModel = async (req, res) => {
  try {
    ...
    // Search for products that match the model (case-insensitive)
    const products = await Product.find({
      model: { $regex: model, $options: 'i' }
    }).select('_id brand model series category basePrice images');
    ...
    console.error('Error finding products by model:', error);
    res.status(500).json({ 
      success: false,
