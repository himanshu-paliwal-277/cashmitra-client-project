...
export const completeEvaluationSchema = {
  body: z.object({
    finalPrice: z.number({ invalid_type_error: 'Final price is required and must be a number' }),
    adjustmentReason: z.string().trim().max(500, 'Adjustment reason must not exceed 500 characters').optional(),
    answers: z.array(z.any()).optional(),
    selectedDefects: z.array(z.any()).optional(),
...
export const completePaymentSchema = {
  body: z.object({
    ...
    transactionId: z.string().trim().max(100, 'Transaction ID must not exceed 100 characters').optional(),
...
export const reEvaluateDeviceSchema = {
  body: z.object({
    ...
    agentNotes: z.string().trim().max(1000, 'Agent notes must not exceed 1000 characters').optional(),
    negotiation: z.number({ invalid_type_error: 'Negotiation must be a number' }).optional(),
  }),
};
