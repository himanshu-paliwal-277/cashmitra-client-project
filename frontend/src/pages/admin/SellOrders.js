...
const SellOrders = () => {
  ...
  const handleAssignOrder = async (orderId, agentId) => {
    ...
    try {
      ...
      const response = await api.put(`/sell-orders/${orderId}/assign-staff`, {
        assignedTo: agentId,
      });
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>
          <ShoppingCart size={32} /> Sell Orders
        </h1>
      </div>
      ...
      <div className={styles.filterSection}>
        ...
        />
        <button onClick={() => setStatusFilter('')} className={styles.filterBtn}>
          <Filter size={16} /> All
        </button>
        <button onClick={() => setStatusFilter('confirmed')} className={styles.filterBtn}>
          Confirmed
        </button>
        <button onClick={() => setStatusFilter('picked')} className={styles.filterBtn}>
          Picked
        </button>
        <button onClick={() => setStatusFilter('paid')} className={styles.filterBtn}>
          Paid
        </button>
      </div>
                    ...
                    {order.assignedTo ? (
                      <div className={styles.agentInfo}>
                        ...
                        {order.assignedTo.phone && (
                          <div className={styles.agentContact}>{order.assignedTo.phone}</div>
                        )}
                      </div>
                    ) : (
                      <>
          ...
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              <section className={styles.section}>
                ...
                <div className={styles.infoGrid}>
                  <div className={styles.infoItem}>
                    <strong>Name:</strong> {selectedOrder.pickup?.address?.fullName || 'N/A'}
                  </div>
                  <div className={styles.infoItem}>
                    <strong>Phone:</strong> {selectedOrder.pickup?.address?.phone || 'N/A'}
                  </div>
                  <div className={styles.infoItem}>
              ...
              <section className={styles.section}>
                ...
                <div className={styles.infoGrid}>
                  <div className={styles.infoItem}>
                    <strong>Product:</strong> {getProductName(selectedOrder)}
                  </div>
                  <div className={styles.infoItem}>
                    <strong>Status:</strong>{' '}
                    <span
                      className={`${styles.statusBadge} ${styles[`status_${selectedOrder.status}`]}`}
                    >
              ...
              <section className={styles.section}>
                ...
                <div className={styles.addressBox}>
                  <p>
                    <strong>{selectedOrder.pickup?.address?.fullName}</strong>
                  </p>
                  ...
                  {selectedOrder.pickup?.slot?.date && (
                    <p>
                      <strong>Slot:</strong> {formatDate(selectedOrder.pickup.slot.date)}{' '}
