...
vendorPermissionSchema.methods.grantPermission = function(menuItem, grantedBy, restrictions = {}) {
...
vendorPermissionSchema.methods.revokePermission = function(menuItem, revokedBy) {
  if (!MENU_ITEMS[menuItem]) {
    throw new Error(`Invalid menu item: ${menuItem}`);
  }
...
vendorPermissionSchema.methods.applyRoleTemplate = function(template, appliedBy) {
  const templates = {
    basic: ['dashboard'],
    sales: ['dashboard', 'sell', 'leads', 'sellOrders', 'buyOrders'],
    inventory: ['dashboard', 'products', 'catalog', 'inventoryApproval'],
    finance: ['dashboard', 'pricing', 'priceTable', 'finance', 'walletPayouts', 'reports'],
...
const VendorPermission = mongoose.model('VendorPermission', vendorPermissionSchema);
