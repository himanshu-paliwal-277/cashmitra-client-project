...
exports.getPartnerPermissions = async (req, res) => {
  try {
    ...
    if (!roleTemplate) {
      roleTemplate = await RoleTemplate.findOne({ name: 'basic', isDefault: true });
      
      // If still no template, create default templates
      if (!roleTemplate) {
        const templates = await RoleTemplate.createDefaultTemplates(req.user.id);
...
exports.getPartnerPermissionsById = async (req, res) => {
  try {
    ...
    if (!permissions) {
      ...
      const newPermissions = await PartnerPermission.createDefaultPermissions(partnerId);
      return res.status(200).json({
        success: true,
        data: newPermissions,
...
exports.createRoleTemplate = async (req, res) => {
  try {
    const { name, displayName, description, color, permissions, features, limits } = req.body;
...
exports.updateRoleTemplate = async (req, res) => {
  try {
    ...
    const { displayName, description, color, permissions, features, limits } = req.body;
    ...
    if (features) roleTemplate.features = { ...roleTemplate.features, ...features };
...
exports.createPermission = async (req, res) => {
  try {
    const { name, displayName, description, category, path, icon, requiredPermission } = req.body;
    ...
    const sanitizedData = {
      ...
      path: sanitizeData(path || `/partner/${name.toLowerCase().replace(/\s+/g, '-')}`),
      icon: sanitizeData(icon || 'Settings'),
      requiredPermission: sanitizeData(requiredPermission || name.toLowerCase().replace(/\s+/g, ''))
    };
