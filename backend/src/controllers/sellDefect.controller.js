...
exports.createDefect = asyncHandler(async (req, res) => {
  ...
  if (existingDefect) {
    throw new ApiError(400, 'Defect with this key already exists for this category');
  }

  // Get next order number for this section
  const lastDefect = await SellDefect.findOne({ categoryId, section }).sort({ order: -1 });
...
exports.getDefectsForCustomer = asyncHandler(async (req, res) => {
  ...
  const groupedDefects = await SellDefect.getGroupedByCategory(productId, [variantId]);
...
exports.updateDefect = asyncHandler(async (req, res) => {
  ...
  const { id } = req.params;
  const { categoryId, section, key, title, icon, delta, order, isActive } = req.body;
...
exports.reorderDefects = asyncHandler(async (req, res) => {
  ...
  if (!productId || !category || !Array.isArray(defectIds)) {
    throw new ApiError(400, 'Product ID, category, and defect IDs array are required');
  }
