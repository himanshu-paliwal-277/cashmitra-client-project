...
const dummyBuyProductData = {
  ...
  images: {
    ...
    gallery: "https://example.com/iphone15-1.jpg,https://example.com/iphone15-2.jpg,https://example.com/iphone15-3.jpg",
  ...
  description: "The iPhone 15 Pro Max represents the pinnacle of smartphone technology with its titanium design, A17 Pro chip, and advanced camera system. Perfect for professionals and enthusiasts who demand the best.",
...
async function testCreateBuyProduct() {
  try {
    ...
    const response = await axios.post(`${BASE_URL}/buy-products`, dummyBuyProductData, {
      headers: {
...
async function runTests() {
  ...
  if (createdProduct && createdProduct.data && createdProduct.data._id) {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/buy-products/${createdProduct.data._id}`);
      
      console.log('✅ SUCCESS: Retrieved Single Buy Product');
      console.log('📊 Response Status:', response.status);
      console.log('📋 Product Details:', JSON.stringify(response.data.data, null, 2));
    } catch (error) {
