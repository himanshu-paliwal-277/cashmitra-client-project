...
async function testCreateSession() {
  try {
    ...
    console.log(
      `Defects Stored: ${response.data.data.assessment.defects.length}`
    );
    console.log(
      `Accessories Stored: ${response.data.data.assessment.accessories.length}`
    );
...
async function testGetSession(sessionId) {
  try {
    ...
    const response = await axios.get(`${BASE_URL}/sell-sessions/${sessionId}`);
