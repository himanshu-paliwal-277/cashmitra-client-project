...
exports.createAccessory = asyncHandler(async (req, res) => {
  ...
  if (existingAccessory) {
    throw new ApiError(400, 'Accessory with this key already exists for this category');
  }

  // Get next order number
  const lastAccessory = await SellAccessory.findOne({ categoryId }).sort({ order: -1 });
...
exports.bulkCreateAccessories = asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(accessories) || accessories.length === 0) {
    throw new ApiError(400, 'Accessories array is required and cannot be empty');
  }
  ...
  if (duplicateKeys.length > 0) {
    throw new ApiError(400, `Duplicate keys found in request: ${duplicateKeys.join(', ')}`);
  }
  ...
  if (existingAccessories.length > 0) {
    ...
    throw new ApiError(400, `Accessories with these keys already exist for this category: ${existingKeys.join(', ')}`);
  }
  ...
  // Bulk insert
  const createdAccessories = await SellAccessory.insertMany(accessoriesToCreate);
...
exports.reorderAccessories = asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(accessoryIds) || accessoryIds.length === 0) {
    throw new ApiError(400, 'Accessory IDs array is required and cannot be empty');
  }
  ...
  if (accessories.length !== accessoryIds.length) {
    throw new ApiError(400, 'Some accessories do not belong to the specified category');
  }
