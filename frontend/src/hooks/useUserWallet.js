...
const useUserWallet = () => {
  ...
  // Add money to wallet
  const addMoney = async (amount, paymentMethod = 'card') => {
    setLoading(true);
    setError(null);
    try {
  ...
  const withdrawMoney = async (amount, bankDetails) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const transferMoney = async (recipientId, amount, note = '') => {
    setLoading(true);
    setError(null);
    try {
  ...
  const downloadStatement = async (startDate, endDate) => {
    setLoading(true);
    setError(null);
    try {
