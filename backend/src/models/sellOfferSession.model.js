...
sellOfferSessionSchema.index({ sessionToken: 1 }, { unique: true, sparse: true });
...
sellOfferSessionSchema.pre('save', function(next) {
  if (this.isModified('answers') || this.isModified('defects') || this.isModified('accessories')) {
    this.computedAt = new Date();
  }
  next();
});

const SellOfferSession = mongoose.model('SellOfferSession', sellOfferSessionSchema);
