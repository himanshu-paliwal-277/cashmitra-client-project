...
const searchProducts = async (req, res) => {
  try {
    const { query, category, condition, minPrice, maxPrice, pincode } = req.query;
    ...
    let partnerIds = [];
    if (pincode) {
      const partners = await Partner.find({ 'address.pincode': pincode, isVerified: true });
      partnerIds = partners.map(partner => partner._id);
      
      if (partnerIds.length === 0) {
        return res.json({ products: [], total: 0, message: 'No partner shops available in your area' });
      }
...
const getProductDetails = async (req, res) => {
  try {
    const product = await BuyProduct.findById(req.params.id)
      .populate('partner', 'shopName address phone rating');
  ...
  try {
    ...
    const cartItemsPromises = cart.items.map(async (item) => {
      ...
      return {
        ...
        subtotal: (product.pricing.discountedPrice || product.pricing.mrp) * item.quantity,
        product: {
          brand: product.brand,
          name: product.name,
    ...
    cart.items = cart.items.filter(item =>
      cartItems.some(cartItem => cartItem && cartItem.productId.toString() === item.productId.toString())
    );
