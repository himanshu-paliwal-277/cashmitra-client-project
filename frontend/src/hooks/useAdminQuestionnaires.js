...
const useAdminQuestionnaires = () => {
  ...
  const editQuestionnaire = useCallback(async (questionnaireId, questionnaireData) => {
    setLoading(true);
    setError(null);
    try {
      const response = await adminService.updateConditionQuestionnaire(questionnaireId, questionnaireData);
      if (response.success && response.data) {
  ...
  const getQuestionnaireByCategory = useCallback((categoryId) => {
  ...
  const categoryHasQuestionnaire = useCallback((categoryId) => {
