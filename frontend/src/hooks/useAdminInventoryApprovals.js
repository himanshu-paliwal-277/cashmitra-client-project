...
const useAdminInventoryApprovals = () => {
  ...
  const updateApprovalStatus = useCallback(
    async (approvalId, status, notes = '') => {
      ...
      setError(null);
      try {
        const response = await adminService.updateInventoryApprovalStatus(
          approvalId,
          status,
          notes
        );
  ...
  // Approve inventory
  const approveInventory = useCallback(
  ...
  // Reject inventory
  const rejectInventory = useCallback(
    async (approvalId, notes = '') => {
  ...
  const reviewInventory = useCallback(
    async (approvalId, notes = '') => {
  ...
  const getApprovalsByPartner = useCallback(
    partnerId => {
