...
const PickupBooking = () => {
  ...
  console.log('PickupBooking - Received data:', { assessmentData, product, priceData, sessionId, selectedVariant });
  ...
  const paymentOptions = [
    { id: 'bank', type: 'bank', label: 'Bank Transfer', icon: '🏦', description: 'Direct bank account transfer' },
    { id: 'upi', type: 'upi', label: 'UPI Payment', icon: '📱', description: 'Pay via UPI apps like GPay, PhonePe' },
    { id: 'cod', type: 'cod', label: 'Cash on Delivery', icon: '💵', description: 'Pay cash when we pick up your device' }
  ];
  ...
  const handlePaymentSelect = (optionId) => {
    setFormData(prev => ({ 
      ...prev, 
  ...
  const handleContinue = async () => {
    if (currentStep === 1) {
      ...
      try {
        ...
        const extractBrandFromProduct = (productData) => {
          if (productData?.tags?.length > 0) {
            // Look for brand in tags (e.g., 'Apple' in ['iPhone', '13 Pro', 'Apple', 'Smartphone'])
            const brandTags = ['Apple', 'Samsung', 'OnePlus', 'Xiaomi', 'Oppo', 'Vivo', 'Realme', 'Huawei'];
            const foundBrand = productData.tags.find(tag => 
        ...
        const orderData = {
          ...
          pickup: {
            address: {
              ...
              street: `${formData.flatNo}, ${formData.locality}${formData.landmark ? `, Near ${formData.landmark}` : ''}`.trim(),
              city: formData.city,
  ...
  const variantLabel = assessmentData?.selectedVariant?.label || selectedVariant?.label || selectedVariant || 'Variant';
        ...
        <div className="booking-steps-section">
            ...
            {currentStep === 3 && (
              <div className="booking-payment-section">
                <div className="booking-payment-group">
                  <div className="booking-input-label">
                    <Wallet size={18} style={{ display: 'inline', marginRight: '8px', verticalAlign: 'middle' }} />
                  ...
                  {paymentOptions.map(option => (
                    ...
                    >
                      ...
                      <div className="booking-payment-details">
                        ...
                        {option.description && <div className="booking-extra-info">{option.description}</div>}
                      </div>
                    </label>
                  ))}
                </div>
                ...
                <div className="booking-payment-note">
                  <div style={{ fontSize: '13px', color: '#6c757d', padding: '12px', background: '#f8f9fa', borderRadius: '8px', borderLeft: '3px solid #667eea' }}>
                    💡 <strong>Secure Payment:</strong> All payment methods are safe and secure. For COD, please keep exact amount ready during pickup.
                  </div>
                </div>
              </div>
            )}
          ...
          {currentStep > 1 && (
            <div className="booking-edit-buttons">
              <button className="booking-edit-btn" onClick={() => handleEdit(1)}>Edit Address</button>
              {currentStep > 2 && <button className="booking-edit-btn" onClick={() => handleEdit(2)}>Edit Pickup</button>}
            </div>
          )}
        </div>
        ...
        <div className="booking-price-summary-section">
          <div className="booking-device-preview">
            ...
            <img 
              ...
              src={product?.data?.images?.[0] || product?.images?.[0] || 'https://via.placeholder.com/140x280?text=Device'} 
              alt={deviceName} 
            />
          ...
          <div className="booking-price-summary">
            ...
            <div className="booking-price-row">
              ...
              <span className="booking-value">{formatPrice(priceData?.quotedPrice || priceData?.basePrice || 1200)}</span>
            </div>
