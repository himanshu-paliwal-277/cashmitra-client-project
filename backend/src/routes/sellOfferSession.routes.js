...
const updateAnswersValidation = [
  ...
  body('answers.*')
    .notEmpty()
    .withMessage('Answer values cannot be empty')
];

const updateDefectsValidation = [
  body('defects')
    .isArray()
    .withMessage('Defects array is required'),
  body('defects.*')
    .isString()
    .withMessage('Each defect must be a string')
];
...
router.put('/:sessionId/accessories', updateAccessoriesValidation, updateAccessories);
router.post('/:sessionId/extend', extendSession);
router.delete('/:sessionId', deleteSession);

// Admin only routes
router.use(authorize('admin'));
router.post('/cleanup-expired', cleanupExpiredSessions);
