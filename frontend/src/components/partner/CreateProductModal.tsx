...
const CreateProductModal: React.FC<CreateProductModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const fetchCategoriesBySuperCategory = async (superCategoryId: string) => {
    try {
      ...
      setFormData((prev: any) => ({ ...prev, categoryId: '' })); // Clear selected category
      ...
      setFormData((prev: any) => {
        ...
        if (name === 'pricing.mrp' || name === 'pricing.discountedPrice') {
          ...
          if (mrp > 0 && discountedPrice > 0 && discountedPrice < mrp) {
            ...
            // If discounted mrp is greater than or equal to MRP, set discount to 0
            newData.pricing.discountPercent = '0';
          } else if (discountedPrice === 0 || val === '') {
            // If discounted mrp is cleared, reset discount percentage
  ...
  // Handle image upload for condition options (specifically for "Excellent" condition)
  ...
  const validateForm = () => {
    ...
    // Check for mrp in first "Excellent" condition option
    const excellentCondition = formData.conditionOptions[0];
    if (!excellentCondition?.mrp || parseFloat(excellentCondition.mrp) <= 0) {
      newErrors['conditionOptions.0.mrp'] = 'Valid mrp is required for Excellent condition';
    }
  ...
  const scrollToError = (fieldName: string) => {
    // Map field names to more user-friendly descriptions
    const fieldDescriptions: { [key: string]: string } = {
      superCategoryId: 'Super Category',
      categoryId: 'Category',
      name: 'Product Name',
      brand: 'Brand',
    ...
    setErrors((prev: any) => ({
      ...prev,
      submit: `Please fill in the required field: ${fieldDescription}`,
    }));

    // Try to scroll to the field
    setTimeout(() => {
      const element = document.querySelector(`[name="${fieldName}"], #image-upload`);
      if (element) {
  ...
  const isFormValid = () => {
    ...
    return (
      ...
      excellentCondition?.mrp &&
      parseFloat(excellentCondition.mrp) > 0 &&
      formData.stock.quantity &&
      parseInt(formData.stock.quantity) >= 0 &&
      excellentCondition?.images &&
      excellentCondition.images.length > 0
    );
  };
    ...
    setFormData({
      ...
      productDetails: {
        ...
        battery: {
          capacity: '',
          type: 'Li-Po',
          charging: { wired: '', wireless: false, fastCharging: false, reverseCharging: false },
        },
        camera: {
          rear: {
            primary: '',
      ...
      trustMetrics: { warranty: '', returnPolicy: '', authenticity: false },
      badges: { qualityChecks: '', warranty: '', refundPolicy: '', assurance: '' },
      paymentOptions: { emiAvailable: false, emiPlans: [], methods: ['Cash', 'UPI', 'Card'] },
      isActive: true,
    });
    setCategories([]);
    setErrors({});
      ...
      <div className="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
              ...
              {openSections.categorySelection && (
                ...
                <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div
                      className={`flex items-center gap-1 ${formData.name.trim() ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {formData.name.trim() ? '✅' : '❌'} Product Name
                    </div>
                    <div
                      className={`flex items-center gap-1 ${formData.brand.trim() ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {formData.brand.trim() ? '✅' : '❌'} Brand
                    </div>
                    <div
                      className={`flex items-center gap-1 ${formData.description.trim() ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {formData.description.trim() ? '✅' : '❌'} Description
                    </div>
                    <div
                      className={`flex items-center gap-1 ${formData.pricing.mrp && parseFloat(formData.pricing.mrp) > 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {formData.pricing.mrp && parseFloat(formData.pricing.mrp) > 0 ? '✅' : '❌'}{' '}
                      MRP
                    </div>
                    <div
                      className={`flex items-center gap-1 ${formData.stock.quantity && parseInt(formData.stock.quantity) >= 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {formData.stock.quantity && parseInt(formData.stock.quantity) >= 0
                        ? '✅'
                        : '❌'}{' '}
                      Stock Quantity
                    </div>
                    <div
                      className={`flex items-center gap-1 ${formData.images.length > 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {formData.images.length > 0 ? '✅' : '❌'} Product Images
                    </div>
                  </div>
                  <div className="mt-2">
                    ...
                    <p className="text-xs text-gray-600 mt-1">
                      ...
                      of 6 required fields completed
                    </p>
                  </div>
                </div>
              )} */}
                    ...
                    <button
                      ...
                      onClick={() =>
                        setFormData((prev: any) => ({
                          ...
                          variants: [
                            ...prev.variants,
                            { storage: '', color: '', mrp: 0, stock: true },
                          ],
                        }))
                      }
                      className="flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200"
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={e => {
                                    ...
                                    // Sync with main pricing if it's the first condition
                                    if (index === 0) {
                                      setFormData((prev: any) => ({
                                        ...prev,
                                        conditionOptions: newConditions,
        ...
        <div className="p-6 border-t border-gray-200 flex justify-end gap-4">
          ...
          <button
            ...
            disabled={loading || !formData.categoryId || !isFormValid()}
