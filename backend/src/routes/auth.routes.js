...
const { validateRequest, validatePasswordStrength } = require('../middlewares/validation.middleware');
...
router.get('/partner/me', protect, asyncHandler(authController.getCurrentPartner));
...
router.put(
  ...
  protect,
  [
    check('name').optional().notEmpty().withMessage('Shop name cannot be empty'),
    check('email').optional().isEmail().withMessage('Please include a valid email'),
    check('phone').optional().isMobilePhone().withMessage('Please include a valid phone number'),
