...
const conditionQuestionnaireSchema = new Schema({
  ...
  category: {
    ...
    enum: {
      values: ['smartphone', 'laptop', 'tablet', 'smartwatch', 'headphones', 'camera', 'gaming', 'general'],
      message: 'Category must be one of: smartphone, laptop, tablet, smartwatch, headphones, camera, gaming, general'
    },
    lowercase: true
  },
  subcategory: {
...
// Static methods
conditionQuestionnaireSchema.statics.findByCategory = function(category, includeInactive = false) {
...
conditionQuestionnaireSchema.statics.findByBrandModel = function(brand, model, category) {
  return this.find({
    $or: [
      { brand: brand, model: model, category: category.toLowerCase() },
      { brand: brand, model: { $exists: false }, category: category.toLowerCase() },
...
conditionQuestionnaireSchema.methods.updateCompletionTime = function(timeInMinutes) {
...
conditionQuestionnaireSchema.methods.duplicate = function(newTitle, createdBy) {
...
module.exports = mongoose.model('ConditionQuestionnaire', conditionQuestionnaireSchema);